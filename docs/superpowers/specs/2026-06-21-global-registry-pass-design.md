...
Each global-registry window receives a compact identity projection of the current saved registry. The saved `registry.json` remains rich enough for UI editing and voice caching, but the prompt omits voice variants, Qwen instructions, hashes, old evidence, and narrative notes. After a successful window, the app immediately merges the returned characters into `registry.json`; the next window sees that updated registry. The prompt treats the existing registry as authoritative and instructs the model not to recreate characters already represented by role id, display name, alias, profile id, or person id. It should return only new characters or genuinely updated existing characters found in the current chapter window.
